...
//    private static byte[] b64;
    ...
    private long roll_hash(int c) {
        ...
        if (roll_state.n == ROLLING_WINDOW)
            roll_state.n = 0;
        ...
        //roll_state.h3 = (roll_state.h3 ^ c) & 0xffffffffL;
...
/*    public char[] snprintf( int size, String format, Object ... args ) {
     ...
     */
    private boolean has_common_substring(byte[] s1, byte[] s2) {
        int i, j;
        int num_hashes;
...
*/
     ...
     */
/*
    private boolean ArrayCompare(byte[] array1, int idx1, byte[] array2, int idx2, int rollingWindow)
    {
...
//        byte[] s1, s2;
        ...
        if (block_size1 != block_size2 &&
                (block_size1 > Long.MAX_VALUE / 2 || block_size1 * 2 != block_size2) &&
        ...
        s1_1 = eliminate_sequences(signature1.getHashPart1());
        s2_1 = eliminate_sequences(signature2.getHashPart1());

        // now break them into the two pieces
//        s1_1 = s1;
//        s2_1 = s2;
        s1_2 = eliminate_sequences(signature1.getHashPart2());
        s2_2 = eliminate_sequences(signature2.getHashPart2());
        ...
        if (block_size1 <= Long.MAX_VALUE / 2) {
            if (block_size1 == block_size2) {
                long score1, score2;
